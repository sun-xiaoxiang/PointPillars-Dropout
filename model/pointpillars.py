...
class PillarEncoder(nn.Module):
    def __init__(self, voxel_size, point_cloud_range, in_channel, out_channel, dropout_p):
        super().__init__()
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar, drop_out):
        '''
        ...
        if drop_out:
            try:
                # print("\n[before dropout] ", features[0][0])
                # print("[before dropout size] ", features.size())
                features = self.dropout(features)
                # print("[after dropout] ", features[0][0])
                # print("[after dropout size] ", features.size())
            except BaseException as e:
                print("[dropout err] ", e)

        # features = self.bn(features)
        features = F.relu(features)  # (p1 + p2 + ... + pb, out_channels, num_points)
...
class PointPillars(nn.Module):
    def __init__(self,
                 ...
                 dropout_p=0):
        super().__init__()
        self.train_dropout = nn.Dropout(p=0.4)
    ...
    def forward(self, batched_pts, mode='test', drop_out=False, batched_gt_bboxes=None,
        ...
        pillar_features = self.pillar_encoder(pillars, coors_batch, npoints_per_pillar, drop_out)
