...
def create_data_info_pkl(data_root, data_type, prefix, label=True, db=False):
    ...
    if data_type=="train_val":
        ids_file = os.path.join(CUR, 'dataset', 'ImageSets_Reduce_500', f'train.txt')
    else:
        ids_file = os.path.join(CUR, 'dataset', 'ImageSets_Reduce_500', f'{data_type}.txt')
...
def main(args):
    ...
    # kitti_val_infos_dict = create_data_info_pkl(data_root, 'train_val', prefix)
