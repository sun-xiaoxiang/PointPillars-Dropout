...
from utils.vis_o3d import vis_kitti_pts
...
class Kitti(Dataset):
    ...
    def __init__(self, data_root, split, pts_prefix='velodyne_reduced'):
        assert split in ['train', 'val', 'trainval', 'test', "train_val"]
    ...
    def __getitem__(self, index):
        ...
        if self.split in ['train', 'trainval']:
            print(os.path.join(self.data_root, "before_aug"))
            vis_kitti_pts(data_dict['pts'], os.path.join(self.data_root, "before_aug"))
            data_dict = data_augment(self.CLASSES, self.data_root, data_dict, self.data_aug_config)
            print(os.path.join(self.data_root, "after_aug"))
            vis_kitti_pts(data_dict['pts'], os.path.join(self.data_root, "after_aug"))
        else:
